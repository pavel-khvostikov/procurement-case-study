...
There is no reliable, traceable relationship between an approved purchase request (PR) and its invoices.

Finance copies the PR code and supplier into the Invoice application, where the code is stored as unvalidated text. Neither system can prove that the PR exists, is approved, or is linked to a given invoice, so reconciliation happens through email, Slack, and spreadsheets.

The prototype will demonstrate that:
...
As an optional extension, the PR application will show PR authors and finance a read-only list of linked invoice statuses. It will not infer that a PR is fully paid because PRs have no amount or currency and further invoices may arrive.
...
- The FastAPI PR backend and Spring Boot Invoice backend have separate APIs and sessions. They share Postgres and the existing `users` table but do not integrate domain data.
...
- Invoice stores nullable `purchase_request_number` as unvalidated text; both `POST /invoice` and `PUT /invoice/{id}` accept fabricated values.
...
- The PR backend creates shared tables through a one-shot job; Invoice relies on Hibernate `ddl-auto:update`. There is no CI or business-logic test coverage.
...
The core changes the PR backend, Invoice backend, and Invoice frontend. The optional status extension also changes the PR frontend. No third service or technology replacement is needed.
...
The existing stack and Docker Compose topology remain. Spring uses its existing `RestClient`; the optional PR outbound call moves `httpx` to runtime dependencies.
...
Docker uses service names such as `http://pr-app-back:8001`; host execution may use `http://localhost:8001`. Both clients have finite connection and response timeouts. Calls happen at request time, so Compose needs no circular startup dependency.
...
New invoices cannot enter the latter two states. Relationship queries and the optional status panel return only validated rows, so a matching legacy string is not silently promoted.
...
| Invoice service unavailable in the PR view | Show “Invoice information temporarily unavailable”; the rest of the PR remains usable. |
| No linked invoices | Show “No linked invoices yet,” not an error or paid state. |
...
For the design PR, this is an acceptance plan rather than executed verification. Each implementation PR will add tests and record the commands and results.
...
| Optional status leaks invoice data | Require the token; omit amounts and attachments; permit author/finance only; distinguish empty from unavailable. |
...
```bash
(cd pr-app/back && uv sync && uv run pytest)
(cd invoice-app/back && mvn test)
(cd pr-app/front && npm ci && npm run build)
(cd invoice-app/front && npm ci && npm run build)
docker compose up --build
```

### End-to-end demonstration

1. As `alice`, create a PR and send it for approval; as `finadmin`, approve it.
2. In Invoice, select that PR without typing its code and create two invoices.
3. Filter by the validated PR code and see both invoices, excluding one linked to another PR.
4. Use the API to show rejection of fabricated and non-approved PRs.
5. Stop the PR backend; existing invoice reads still work while new links fail clearly.
6. As `alice`, view the linked invoice statuses; stop the Invoice backend and see only that panel degrade.
...
| Pull request | Scope |
|---|---|
| 1. Design proposal | This document only. |
| 2. Trusted PR–invoice relationship | PR read contract; Invoice client and save invariant; validation provenance; exact one-to-many query; create/edit selector and failure states; configuration; backend tests. |
| 3. Requester invoice-status visibility | Reverse read contract; author/finance authorization; degradable PR detail panel; tests; final documentation update. |
...
The design PR does not claim completed implementation. After the feature PRs, the branch will run with:

```bash
docker compose up --build
```

The documented ports remain unchanged, and the six-step scenario above is the acceptance demonstration.
